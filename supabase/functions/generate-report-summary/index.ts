...
const normalizeKeyPoints = (value)=>{
  ...
  if (typeof value === "string" && value.trim()) {
    return value.split(/\n|•|-/).map((item)=>item.replace(/^\s*[-•]\s*/, "").trim()).filter((item)=>item.length > 0);
  }
  return [];
};
const normalizeKeyMetrics = (value)=>{
  if (Array.isArray(value)) {
    return value.map((item)=>{
      ...
      if (item && typeof item === "object") {
        ...
        const label = typeof candidate.label === "string" ? candidate.label.trim() : typeof candidate.metric === "string" ? candidate.metric.trim() : "";
        const valueText = typeof candidate.value === "string" ? candidate.value.trim() : "";
        const trend = typeof candidate.trend === "string" ? candidate.trend.trim() : typeof candidate.description === "string" ? candidate.description.trim() : "";
...
const truncateContent = (content, limit = 12000)=>{
  if (content.length <= limit) {
    return content;
  }
  return `${content.slice(0, limit)}...`; // Append ellipsis to indicate truncation
};
const buildPrompt = (reportContent, sourceDescriptor, options = {})=>{
  const hints = [];
  ...
  const hintsBlock = hints.length ? `Du kan använda följande ledtrådar om de verkar rimliga:\n${hints.map((hint)=>`- ${hint}`).join("\n")}\n\n` : "";
  const objectives = [
...
const fetchDocumentContent = async (documentId)=>{
  ...
  const { data: document, error: documentError } = await supabase.from("chat_documents").select("id, name, status").eq("id", documentId).single();
  if (documentError || !document) {
    console.warn("Failed to fetch chat document", {
      documentId,
      error: documentError
    });
    return {
  ...
  if (document.status !== "processed") {
    console.warn("Document not processed", {
      documentId,
  ...
  const { data: chunks, error: chunkError } = await supabase.from("chat_document_chunks").select("content, chunk_index").eq("document_id", documentId).order("chunk_index", {
    ascending: true
  }).limit(200);
...
const fetchSourceContent = async (url)=>{
  try {
    ...
    console.warn("Error fetching source URL", {
      url,
      error
    });
    return null;
...
serve(async (req)=>{
  ...
  if (!sourceContent) {
    return new Response(JSON.stringify({
      success: false,
      error: "Ingen rapporttext tillgänglig"
    }), {
      status: 400,
      headers: {
        ...corsHeaders,
  ...
  const sourceDescriptor = sourceUrl ? `från ${sourceUrl}` : resolvedDocumentName ? `från dokumentet ${resolvedDocumentName}` : payload?.source_type === "document" ? "från det uppladdade dokumentet" : "inklistrad text";
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        ...
        reasoning: {
          effort: "none" // bättre struktur, mindre hallucinationer
        },
        text: {
        ...
        input: [
          {
            role: "system",
            content: "Du är en erfaren finansanalytiker som levererar koncisa, professionella rapportsammanfattningar på svenska. Du returnerar alltid strikt giltig JSON utan någon text utanför JSON-strukturen."
          },
          {
            role: "user",
            content: prompt
    ...
    if (!response.ok) {
      ...
      return new Response(JSON.stringify({
        success: false,
        error: "AI-modellen kunde inte generera en rapport"
      }), {
        status: 502,
        headers: {
          ...corsHeaders,
    ...
    const content = data.output_text || data.output?.find((o)=>o.type === "message")?.content?.[0]?.text || data.choices?.[0]?.message?.content || "";
    if (!content) {
    ...
    const parsedCompanyName = typeof parsed?.company_name === "string" ? parsed.company_name.trim() : "";
    const parsedReportTitle = typeof parsed?.report_title === "string" ? parsed.report_title.trim() : "";
    const summary = typeof parsed?.summary === "string" ? parsed.summary.trim() : "";
    const keyPoints = normalizeKeyPoints(parsed?.key_points);
    const keyMetrics = normalizeKeyMetrics(parsed?.key_metrics);
    const ceoCommentary = typeof parsed?.ceo_commentary === "string" ? parsed.ceo_commentary.trim() : "";
    ...
    if (!summary) {
      return new Response(JSON.stringify({
        success: false,
        error: "Sammanfattningen kunde inte skapas"
      }), {
        status: 502,
        headers: {
          ...corsHeaders,
    ...
    const { data: insertedReport, error: insertError } = await supabase.from("discover_report_summaries").insert({
      id: reportId,
    ...
    if (insertError || !insertedReport) {
      ...
      return new Response(JSON.stringify({
        success: false,
        error: "Kunde inte spara rapporten"
      }), {
        status: 500,
        headers: {
          ...corsHeaders,
    ...
    const responseReport = {
      ...
      sourceType: insertedReport.source_type ?? undefined,
    ...
    return new Response(JSON.stringify({
      success: false,
      error: "Internt fel vid generering"
    }), {
      status: 500,
      headers: {
        ...corsHeaders,
