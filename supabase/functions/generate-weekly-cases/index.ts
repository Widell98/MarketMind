...
const buildPitchIntro = (
  ...
  const desiredSentenceCount = sentences.length >= 3
    ? 3
    : sentences.length >= 2
      ? 2
      : 1;

  const assembled = sentences.slice(0, desiredSentenceCount).join(' ');
  if (assembled.length <= 480) {
    return assembled;
  }

  return `${assembled.slice(0, 477).trimEnd()}...`;
};
...
const sanitizeCaseData = (rawCase: any) => {
  ...
  return {
    ...
    ai_intro: buildPitchIntro(descriptionRaw, longDescription),
    sector,
