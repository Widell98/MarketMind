...
-- 1) Subscribers: restrict SELECT to user_id only
...
BEGIN
  FOR p IN (
    ...
    EXECUTE format('DROP POLICY %I ON public.subscribers', p.policyname);
  END LOOP;
END $$;
...
-- Do not create INSERT/UPDATE/DELETE policies; absence denies direct writes. All changes must go via SECURITY DEFINER functions.


-- 3) security_audit_log: remove permissive INSERT policy; rely on SECURITY DEFINER function for inserts
...
BEGIN
  FOR p IN (
    SELECT policyname FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'security_audit_log' AND cmd = 'insert'
  ) LOOP
    EXECUTE format('DROP POLICY %I ON public.security_audit_log', p.policyname);
  END LOOP;
END $$;

-- Keep existing SELECT policy (admins only) intact; no INSERT policy is needed


-- 4) Add enhanced role assignment function with audit logging (idempotent)
