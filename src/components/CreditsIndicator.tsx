...
const CreditsIndicator = ({ type = 'ai_message', showUpgrade = true, onUpgrade }: CreditsIndicatorProps) => {
  const { subscription, usage, getRemainingUsage } = useSubscription();
  const { user } = useAuth();
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {showUpgrade && remaining === 0 && onUpgrade && (
        <Button 
          size="sm" 
          onClick={onUpgrade}
