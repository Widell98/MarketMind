      ...
      <CardContent>
        <div className="space-y-3">
          {latestCases.map((stockCase, index) => (
            <div
              key={stockCase.id}
              className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-all duration-200 cursor-pointer border border-transparent hover:border-blue-200 dark:hover:border-blue-800"
              onClick={() => navigate(`/stock-cases/${stockCase.id}`)}
            >
              {/* Ranking Badge - Smaller and more elegant */}
              <div className="flex-shrink-0">
                <div className="w-6 h-6 rounded-full bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center text-white font-semibold text-xs shadow-sm">
                  {index + 1}
                </div>
              </div>
