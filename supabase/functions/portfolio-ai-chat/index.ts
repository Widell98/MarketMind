...
serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({
      error: error.message,
      details: 'Misslyckades med att bearbeta AI-chattförfrågan'
    }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
...
function buildSystemPrompt(riskProfile: any, portfolio: any, shouldRecommend: boolean, contextType: string): string {
  if (contextType === 'initial_strategy') {
    return `Du är FinLyze, en AI-driven finansiell strateg baserad på GPT-4o. Ditt jobb är att ta användarens investeringsinformation och returnera en personlig investeringsstrategi och portföljplan.
...
- Nuvarande portfölj: ${portfolio && portfolio.portfolio_holdings ? portfolio.portfolio_holdings.map(h => `${h.stock_name} (${h.stock_symbol})`).join(', ') : 'Ingen'}
...
5. Avsluta med att påminna att detta inte är personlig rådgivning, utan modellbaserad vägledning`;
  }

  return `Du är FinCoach, en GPT-4o-baserad AI-rådgivare. Användaren har skapat en initial portfölj. Din uppgift är att förbättra och optimera den i pågående dialog.

Beteende:
- Ställ frågor före du ger förslag
- Analysera diversifiering och risk
- Anpassa råd till användarens profil
- Beskriv för- och nackdelar
- Ta hänsyn till användarens nuvarande innehav i varje analys och diskussion

Användarens portfölj:
${portfolio && portfolio.portfolio_holdings ? portfolio.portfolio_holdings.map(h => `- ${h.stock_name} (${h.stock_symbol}): ${h.quantity} st`).join('\n') : 'Ingen'}
...
- Justera sektorfördelning
- Simulera konservativa vs. aggressiva modeller

Begränsning: Detta är modellbaserad vägledning, inte finansiell rådgivning.`;
}
