      ...
      <CardContent>
        <div className="space-y-4">
          {latestCases.map((stockCase, index) => (
            <div
              key={stockCase.id}
              className="flex items-center gap-4 p-4 rounded-xl hover:bg-white/50 dark:hover:bg-gray-800/50 transition-all duration-200 cursor-pointer border border-transparent hover:border-blue-200 dark:hover:border-blue-800 hover:shadow-md group"
              onClick={() => navigate(`/stock-cases/${stockCase.id}`)}
            >
              {/* Ranking Badge - Enhanced */}
              <div className="flex-shrink-0">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center text-white font-semibold text-sm shadow-lg group-hover:shadow-xl transition-shadow">
                  {index + 1}
                </div>
              </div>
              ...
              <div className="flex-1 min-w-0">
                <h4 className="font-bold text-gray-900 dark:text-gray-100 truncate text-base group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
